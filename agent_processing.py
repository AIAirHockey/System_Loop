...
def get_mallet():
    ...
    try:
        ...
        lines = data.split('\n')
        latest_msg = "".join(lines)
    except UnicodeDecodeError:
        pass

    if latest_msg:
        try:
            parsed_data = json.loads(latest_msg)

            mallet_pos = np.array(parsed_data["pos"])
            mallet_vel = np.array(parsed_data["vel"])
            mallet_acc = np.array(parsed_data["acc"])
            return mallet_pos, mallet_vel, mallet_acc

        except json.JSONDecodeError :
            print("JSON decode error", latest_msg)
        except KeyError:
            print("JSON key error", latest_msg)
...
def convert_to_standard_types(obj):
    ...
    if isinstance(obj, (np.float32, np.float64)):  # Convert NumPy float to Python float
        return float(obj)
    else:
        return obj  # Return as is if already a standard type

def listen_for_input():
    """Runs in a separate thread to get user input and add it to the queue."""
    user_input = input("\n stop and or change modes [enter]")  # Get user input
    input_queue.put(user_input)  # Store input in the queue

def mallet_calibration():
...
def generate_top_down(pos):
    pos = np.array([1.9885 - pos[0], 0.9905 - pos[1]])
    top_down_image = np.ones((int(0.9905 * 500), int(1.9885 * 500), 3), dtype=np.uint8) * 255

    x_img = int(pos[0] * 500)  # scale factor for x
    y_img = int((0.9905 - pos[1]) * 500)  # invert y-axis for display
    cv2.circle(top_down_image, (x_img, y_img), int(0.101553/2* 500), (0, 255, 0), -1)
    cv2.imshow("top_down_table", top_down_image)
    cv2.waitKey(1)

if __name__ == "__main__":
    """
    puck_pos = np.array([1.64, 0.34])
    puck_vel = np.array([0.23, -0.2])
    op_mallet_pos = np.array([0.3, 0.43])
    op_mallet_vel = np.array([0.03, 0.1])
    mallet_pos = np.array([1.8, 0.6])
    mallet_vel = np.array([0.01, -0.3])
    mallet_acc = np.array([0,0])
    take_action(puck_pos, puck_vel, op_mallet_pos, op_mallet_vel, mallet_pos, mallet_vel, mallet_acc, False)"
    """
        ...
        if mode == 1:
            ...
            while True:
                if time.time() - current_time > 0.5:
                    ...
                    if not input_queue.empty():
                        input_queue.get()
                        data = update_path(pos, vel, acc, xf[i], Vo, True)
                        idleing = True
                    else:
                        data = update_path(pos, vel, acc, xf[i], Vo)
                    ser.write(b'\n' + data + b'\n')
