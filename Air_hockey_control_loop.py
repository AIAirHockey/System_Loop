...
def acquire_images(cam, print_values, visual):
    # Begin image acquisition
    if ap.ser.in_waiting:
        ap.ser.read(ap.ser.in_waiting).decode('utf-8')
    ap.ser.write(b'1')

    ap.mallet_calibration()
    ...
    try:
        while True:
            ...
            got_mallet = False
            while not got_mallet:
                pos, vel, acc, got_mallet = track.get_mallet()
            got_mallet = True

            ap.ser.write("got_init\n".encode())
