...
class PuckTracker:
    def __init__(self):
        ...
        self.mallet_pos = np.zeros((2,))
        self.mallet_vel = np.zeros((2,))
        self.mallet_acc = np.zeros((2,))
    ...
    def generate_top_down_view(self):
        ...
        x_img = int(m_pos[0] * 500)  # scale factor for x
        y_img = int((0.9905 - m_pos[1]) * 500)  # invert y-axis for display
            ...
            if hierarchy[i][2] == -1 and len(contour) > 20:
                ...
                if M['m00'] != 0:
                    ...
                    if mask[img_pos_int[1], img_pos_int[0]] == 0 and mallet_pos is None:
                        ...
                        #print("A")
                        puck_pos = self.fill_circle(points_3D[:-1], points_3D[-1:].squeeze()) #, centroid)
                        #print(puck_pos)
    ...
    def get_mallet(self):
        mp, mv, ma = ap.get_mallet()
        if mp is not None and mv is not None and ma is not None:
            self.mallet_pos = mp
            self.mallet_vel = mv
            self.mallet_acc = ma
            return mp, mv, ma, True
